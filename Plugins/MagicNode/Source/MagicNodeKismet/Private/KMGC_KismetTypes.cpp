...
UMGC_SemanticDB::UMGC_SemanticDB()
{
    ...
    KDefinition.Info = TEXT("Microsoft Specific.\nUse __declspec(align(#)) to precisely control the alignment of "
                            "user-defined data (for example, static allocations or automatic data in a function).");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("alignof");
    KDefinition.Info =
        TEXT("C++11 introduces the alignof operator that returns the alignment, in bytes, of the specified type.");
    KeywordDefinitions.Add(Keyword, KDefinition);
    ...
    Keyword = TEXT("auto");
    KDefinition.Info = TEXT("For variables, specifies that the type of the variable that is being declared will be "
                            "automatically deduced from its initializer.\nFor functions, specifies that the return "
                            "type is a trailing return type or will be deduced from its return statements.");
    KeywordDefinitions.Add(Keyword, KDefinition);
    ...
    Keyword = TEXT("break");
    KDefinition.Info = TEXT(
        "Causes the enclosing for, range-for, while or do-while loop or switch statement to terminate.\nUsed when it "
        "is otherwise awkward to terminate the loop using the condition expression and conditional statements.");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("case");
    KDefinition.Info = TEXT(
        "The body of a switch statement may have an arbitrary number of case: labels, as long as the values of all "
        "expressions are unique:\n\nswitch(1)\n{\n\tcase 1 : Log('1');\n\t\t\tbreak;\n\tcase 2 : Log('2');\n}");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("catch");
    KDefinition.Info = TEXT("Associates one or more exception handlers (catch-clauses) with a compound "
                            "statement.\n\ntry { /*...*/ } catch (const std::exception &e) { /*...*/ }.\nShouldn't be "
                            "used in Unreal code as by default Unreal has exceptions disabled.");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("char");
    KDefinition.Info = TEXT("Type for character representation which can be most efficiently processed on the target "
                            "system.\nMultibyte characters strings use this type to represent text units.");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("char16_t");
    KDefinition.Info = TEXT("Type for UTF-16 character representation, required to be large enough to represent any "
                            "UTF-16 code unit (16 bits).");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("char32_t");
    KDefinition.Info = TEXT("Type for UTF-32 character representation, required to be large enough to represent any "
                            "UTF-32 code unit (32 bits).");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("class");
    KDefinition.Info = TEXT("Declaration of a class.\nClasses and structs are user-defined types, defined by "
                            "class-specifier, which appears in specifier of the declaration syntax.\nThe class "
                            "specifier has the following syntax:\n\nclass-keyword attr class-head-name base-clause { "
    ...
    KDefinition.Info = TEXT("A concept is a named set of requirements. The definition of a concept must appear at "
    ...
    Keyword = TEXT("const");
    KDefinition.Info =
        TEXT("Appear in any type specifier, including decl-specifier-seq of declaration grammar, to specify "
             "constness:\n\nconst object - an object whose type is const-qualified, or a non-mutable subobject of a "
             "const object.\nSuch object cannot be modified, attempt to do so directly is a compile-time error, and "
             "attempt to do so indirectly results in undefined behavior.");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("consteval");
    KDefinition.Info = TEXT("Specifies that a function is an immediate function, that is, every call to the function "
                            "must produce a compile-time constant expression.");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("constexpr");
    KDefinition.Info = TEXT(
        "The constexpr specifier declares that it is possible to evaluate the value of the function or variable at "
        "compile time.\nIt may not be applied to destructors, allocation functions, or deallocation functions.");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("const_cast");
    KDefinition.Info = TEXT("const_cast makes it possible to form a reference or pointer to non-const type that is "
                            "actually referring to a const object\nor a reference or pointer to non-volatile type that "
                            "is actually referring to a volatile object.");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("continue");
    KDefinition.Info = TEXT("Causes the remaining portion of the enclosing for, range-for, while or do-while loop body "
                            "to be skipped.\nUsed when it is otherwise awkward to ignore the remaining portion of the "
                            "loop using conditional statements.");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("decltype");
    KDefinition.Info =
        TEXT("Inspects the declared type of an entity or the type and value category of an expression.\n'decltype' is "
             "useful when declaring types that are difficult or impossible to declare using standard notation,\nlike "
             "lambda-related types or types that depend on template parameters.");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("default");
    KDefinition.Info =
        TEXT("The body of a switch statement may have an arbitrary number of case: labels.\nAt most one default: label "
             "may be present. If condition evaluates to the value that doesn't match any of the case: labels, and the "
             "default: label is present,\ncontrol is transferred to the statement labeled with the default: label.");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("delete");
    KDefinition.Info = TEXT("Destroys object(s) previously allocated by the new expression and releases obtained "
                            "memory area.\nAvoid using this in Unreal UObjects since 'Garbage Collection' should take "
    ...
    KDefinition.Info =
        TEXT("Safely converts pointers and references to classes up, down, and sideways along the inheritance "
             "hierarchy tree.\nIn Unreal Engine consider making use of 'Cast<type>(...)' instead whenever possible.");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("else");
    KDefinition.Info = TEXT("if statement - Conditionally executes another statement.\nUsed where code needs to be "
                            "executed based on a run-time or compile-time condition.");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("enum");
    KDefinition.Info =
        TEXT("Declaration of an enumeration type - An enumeration is a distinct type whose value is restricted to a "
             "range of values, which may include several explicitly named constants (enumerators).\nThe values of the "
             "constants are values of an integral type known as the underlying type of the enumeration.");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("explicit");
    KDefinition.Info =
        TEXT("The explicit specifier may only appear within the decl-specifier-seq of the declaration of a constructor "
             "or conversion function within its class definition.\nSpecifies that a constructor or conversion function "
             "is explicit, that is, it cannot be used for implicit conversions and copy-initialization.");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("export");
    KDefinition.Info =
        TEXT("Used to mark a template definition exported, which allows the same template to be declared, but not "
             "defined, in other translation units.\nMarks a declaration, a group of declarations, or another module as "
             "exported by the current module.\nIn Unreal you usually want to use the 'MODULENAME_API' macro instead.");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("extern");
    KDefinition.Info =
        TEXT("Language linkage - Provides for linkage between modules written in different programming languages.\nSee "
             "more info at  https://en.cppreference.com/w/cpp/language/language_linkage");
    KeywordDefinitions.Add(Keyword, KDefinition);
    ...
    Keyword = TEXT("for");
    KDefinition.Info =
        TEXT("for loop - Executes a for loop over a range. Used as a more readable equivalent to the traditional for "
             "loop operating over a range of values, such as all elements in a container:\n\nfor (int32 I = 0; I < "
             "MyArray.Num(); I++) {\n\t\tif (I <= 9) {\n\t\t\tMyArray[I]->CallFunction();\n\t\t}\n}");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("friend");
    KDefinition.Info = TEXT(
        "friend declaration - The friend declaration appears in a class body and grants a function or another "
        "unrelated class access\nto private and protected members of the class where the friend declaration appears.");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("goto");
    KDefinition.Info = TEXT("Transfers control unconditionally.\nUsed when it is otherwise impossible to transfer "
                            "control to the desired location using other statements (very rare occasions).");
    KeywordDefinitions.Add(Keyword, KDefinition);
    ...
    Keyword = TEXT("inline");
    KDefinition.Info =
        TEXT("The inline specifier, when used in a function's decl-specifier-seq, declares the function to be an "
             "inline function.\nA function defined entirely inside a class/struct/union definition, is implicitly an "
             "inline function.\nInstead of executing the function call CPU instruction to transfer control to the "
             "function body, a copy of the function body is executed without generating the call.\nThis avoids "
             "overhead created by the function call, but it may result in a larger executable as the code for the "
             "function has to be repeated multiple times.");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("int");
    KDefinition.Info = TEXT("Native Type.\n\nBasic integer type. If no length modifiers are present, it's guaranteed "
                            "to have a width of at least 16 bits.\nHowever, on 64 bit systems it is almost exclusively "
                            "guaranteed to have width of at least 32 bits.\n\nMax value:		2147483647");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("long");
    KDefinition.Info = TEXT("long modifier - Target type will have width of at least 32 bits.\nlong long - Target type "
                            "will have width of at least 64 bits.\nFor more info see 'Fundamental Types' at  "
                            "https://en.cppreference.com/w/cpp/language/types");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("mutable");
    KDefinition.Info = TEXT(
        "Applies to non-static class members of non-reference non-const type and specifies that the member does not "
        "affect the externally visible state of the class.\nMutable members of const class instances are modifiable.");
    KeywordDefinitions.Add(Keyword, KDefinition);
    ...
    KDefinition.Info = TEXT("Namespaces provide a method for preventing name conflicts in large projects.\nSymbols "
                            ...
                            "with the same name are allowed.\nAll declarations within those blocks are declared in the "
                            "named scope.\nYou cannot create Unreal types inside namespaces, i.e: UCLASS().");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("new");
    KDefinition.Info = TEXT("Creates and initializes objects with dynamic storage duration, that is, objects whose "
                            "lifetime is not limited by the scope in which they were created.\nDo not use 'new' to "
                            "create UObject instances, use 'NewObject<type>(...)' functions for that.");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("noexcept");
    KDefinition.Info = TEXT("Specifies whether a function could throw exceptions.\nIn general, try/catch exceptions in "
                            "Unreal are disabled by default (don't use).");
    KeywordDefinitions.Add(Keyword, KDefinition);
    ...
    KDefinition.Info =
        TEXT("invalid pointer (literal) - The keyword nullptr denotes the pointer literal. It is a prvalue of type "
    ...
    KDefinition.Info = TEXT("operator overloading - Customizes the C++ operators for operands of user-defined types "
    ...
    KDefinition.Info =
        TEXT("Specifies that a virtual function overrides another virtual function.\nIn a member function declaration "
             "or definition, 'override' ensures that the function is virtual and is overriding a virtual function from "
    ...
    KDefinition.Info = TEXT("In a member-specification of a class/struct or union, define the accessibility of "
    ...
    KDefinition.Info = TEXT("The storage class specifiers are a part of the decl-specifier-seq of a name's declaration "
                            "syntax. (C++17 deprecated)");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("reinterpret_cast");
    KDefinition.Info =
        TEXT("Converts between types by reinterpreting the underlying bit pattern.\nIt is purely a compile-time "
             "directive which instructs the compiler to treat expression as if it had the type new_type.");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("requires");
    KDefinition.Info = TEXT("The keyword 'requires' is used to introduce a requires-clause, which specifies "
                            "constraints on template arguments or on a function declaration.");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("return");
    KDefinition.Info =
        TEXT("return statement - Terminates the current function and returns the specified value (if any) to its "
             "caller.\nAny for loops within current function will be interrupted and function exited.");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("signed");
    KDefinition.Info = TEXT("Target type will have signed representation (this is the default if omitted). I.e: can "
                            "represent negative numbers, below zero.");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("short");
    KDefinition.Info = TEXT("Native Type.\n\nTarget type will be optimized for space and will have width of at least "
                            "16 bits.\n\nMax value:		32767");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("sizeof");
    KDefinition.Info = TEXT("Queries size of the object or type.\nUsed when actual size of the object must be known "
                            "for next expression executed.");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("static");
    KDefinition.Info =
        TEXT("Inside a class definition, the keyword static declares members that are not bound to any "
             "instances.\nThink of it as immutable 'global' declaration that can be accessed by any foreign class "
    ...
    Keyword = TEXT("struct");
    KDefinition.Info = TEXT("Classes and structs are user-defined types, defined by class-specifier, which appears in "
    ...
    KDefinition.Info =
        TEXT("A template is a C++ entity that defines one of the following:\n\nA family of classes (class template), "
    ...
    Keyword = TEXT("this");
    KDefinition.Info = TEXT("this pointer - The keyword this is a 'prvalue' expression whose value is the memory "
                            "address of the object itself, on which the member function is being called.");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("throw");
    KDefinition.Info =
        TEXT("Signals an erroneous condition and executes an error handler.\nUnreal has a library to handle exceptions "
             "which should be used instead, check:\n\n https://docs.unrealengine.com/en-us/Programming/Assertions");
    KeywordDefinitions.Add(Keyword, KDefinition);
    ...
    Keyword = TEXT("try");
    KDefinition.Info = TEXT("try/catch expression - Associates one or more exception handlers (catch-clauses) with a "
                            "compound statement.\n\ntry { /*...*/ } catch (const std::exception &e) { /*...*/ "
    ...
    KDefinition.Info =
        TEXT("Creates an alias that can be used anywhere in place of a (possibly complex) existing type name.");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("typeid");
    KDefinition.Info = TEXT("Queries information of a type.\nUsed where the dynamic type of a polymorphic object must "
                            "be known and for static type identification.");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("typename");
    KDefinition.Info = TEXT("In a template declaration, typename can be used as an alternative to class to declare "
                            "type template parameters and template parameters.");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("union");
    KDefinition.Info =
        TEXT("A union is a special class type that can hold only one of its non-static data members at a time.\nThe "
             "class specifier for a union declaration is similar to class or struct declaration.");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("using");
    KDefinition.Info =
        TEXT("Introduces a name that is defined elsewhere into the declarative region where this using-declaration "
    ...
    KDefinition.Info = TEXT("The virtual specifier specifies that a non-static member function is virtual and supports "
                            "dynamic dispatch.\nIt may only appear in the decl-specifier-seq of the initial "
                            "declaration of a non-static member function.");
    KeywordDefinitions.Add(Keyword, KDefinition);
    ...
    KDefinition.Info = TEXT("An object whose type is volatile-qualified, or a subobject of a volatile object, or a "
                            "mutable subobject of a const-volatile object.");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("wchar_t");
    KDefinition.Info = TEXT("(TCHAR) Type for wide character representation.\nRequired to be large enough to represent "
                            "any supported character code point (32 bits on systems that support Unicode).");
    KeywordDefinitions.Add(Keyword, KDefinition);

    Keyword = TEXT("while");
    KDefinition.Info = TEXT("while loop - Executes a statement repeatedly, until the value of condition becomes false. "
    ...
    Keyword = TEXT("float");
    TDefinition.Info = TEXT("Blueprint Type.\n\nSingle precision floating point type. Usually IEEE-754 32 bit floating "
                            "point type.\n\nMax value:		3.40282 E+38");
    TypeDefinitions.Add(Keyword, TDefinition);

    Keyword = TEXT("double");
    TDefinition.Info = TEXT("Native Type.\n\nDouble precision floating point type. Usually IEEE-754 64 bit floating "
                            "point type.\n\nMax value:		1.79769 E+308");
    TypeDefinitions.Add(Keyword, TDefinition);

    Keyword = TEXT("uint8");
    TDefinition.Info = TEXT("Blueprint Type.\n\nTarget type will be optimized for space and will have width of at "
                            "least 8 bits.\n\nMax value:		0  to  255");
    TypeDefinitions.Add(Keyword, TDefinition);

    Keyword = TEXT("uint16");
    TDefinition.Info = TEXT("Native Type.\n\nSigned 'short' type.\nTarget type will be optimized for space and will "
                            "have width of at least 16 bits.\n\nMax value:		0  to  65535");
    TypeDefinitions.Add(Keyword, TDefinition);
    ...
    Keyword = TEXT("int8");
    TDefinition.Info = TEXT("Native Type.\n\nTarget type will be optimized for space and will have width of at least 8 "
    ...
    TDefinition.Info = TEXT("MGC Type.\nMagic Node Scripts automatically converts this type to 'bool'\n when compiled "
                            "down to Unreal C++ Classes:\n\nCapable of holding one of the two values: true or "
    ...
    TDefinition.Info = TEXT("MGC Type.\nMagic Node Scripts automatically converts this type to 'int16'\n when compiled "
                            "down to Unreal C++ Classes:\n\nSigned 'short' type.\nTarget type will be optimized for "
                            "space and will have width of at least 16 bits.\n\nMax value:		-32768  to  32767");
    TypeDefinitions.Add(Keyword, TDefinition);

    Keyword = TEXT("FInt");
    TDefinition.Info =
        TEXT("MGC Type.\nMagic Node Scripts automatically converts this type to 'int32'\n when compiled down to Unreal "
             "C++ Classes:\n\nSigned 'int' type.\n\nMax value:		-2147483648  to  2147483647");
    TypeDefinitions.Add(Keyword, TDefinition);
    ...
    TDefinition.Info = TEXT("Blueprint Type.\n\nTransform composed of Scale, Rotation (quaternion), and "
    ...
    Keyword = TEXT("TSet");
    TDefinition.Info =
        TEXT("Blueprint Type.\n\nA set with an optional KeyFuncs parameters for customizing how the elements are "
             "compared and searched.\nE.g: You can specify a mapping from elements to keys if you want to find "
             "elements by specifying a subset of\nthe element type.  It uses a TSparseArray of the elements, and also "
             "links the elements into a hash with a\nnumber of buckets proportional to the number of elements.  "
             "Addition, removal, and finding are O(1).");
    TypeDefinitions.Add(Keyword, TDefinition);

    Keyword = TEXT("TMap");
    TDefinition.Info = TEXT("Blueprint Type.\n\nA Blueprintable key/value pair - specialized type that only allows a "
                            "single value associated with each unique key.");
    TypeDefinitions.Add(Keyword, TDefinition);

    Keyword = TEXT("TArray");
    TDefinition.Info =
        TEXT("Blueprint Type.\n\nA dynamically sized array of typed elements. Makes the assumption that your elements "
    ...
    TDefinition.Info = TEXT(
        "Native Type.\n\nTSharedPtr is a non-intrusive reference-counted authoritative object pointer. This shared "
        "pointer\nwill be conditionally thread-safe when the optional Mode template argument is set to ThreadSafe.");
    TypeDefinitions.Add(Keyword, TDefinition);

    Keyword = TEXT("TSharedRef");
    TDefinition.Info = TEXT("Native Type.\n\nTSharedRef is a non-nullable, non-intrusive reference-counted "
                            "authoritative object reference.\nThis shared reference will be conditionally thread-safe "
    ...
    Keyword = TEXT("IMGC");
    MDefinition.Info = TEXT("MGC Macro.\n\nRegisters the 'Execute' function signature that allows this Node to be "
    ...
    MDefinition.Info = TEXT("MGC Macro.\n\nEarliest call of a 'Latent Runtime Node's routines,\nif this Script has "
                            "subscribed to be latent.\n(executes only once)");
    MacroDefinitions.Add(Keyword, MDefinition);

    Keyword = TEXT("START");
    MDefinition.Info = TEXT("MGC Macro.\n\nCalled from 'Latent Runtime Node's routines once subscription was "
                            "complete,\nif this Script has subscribed to be latent.\n(executes only once)");
    MacroDefinitions.Add(Keyword, MDefinition);

    Keyword = TEXT("UPDATE");
    MDefinition.Info = TEXT("MGC Macro.\n\nCalled from 'Latent Runtime Node's routines,\nif this Script has subscribed "
                            "to be latent.\n(executes every tick)");
    MacroDefinitions.Add(Keyword, MDefinition);

    Keyword = TEXT("STOP");
    MDefinition.Info = TEXT("MGC Macro.\n\nCalled from 'Latent Runtime Node's routines once this Script is set to be "
                            "destroyed,\nif this Script has subscribed to be latent.\n(executes only once)");
    MacroDefinitions.Add(Keyword, MDefinition);

    Keyword = TEXT("FSelf");
    MDefinition.Info = TEXT("MGC Type.\nMagic Node Scripts automatically convert this type to {Script Class}\n when "
                            "compiled down to Unreal C++ Classes:\n\nReference to 'this' class (in text form, not a "
                            "pointer).\nThis type doesn't exist in Unreal C++ space.");
    MacroDefinitions.Add(Keyword, MDefinition);

    Keyword = TEXT("Ensure");
    MDefinition.Info = TEXT("MGC Macro.\n\nChecks if a pointer to an object is valid,\npreventing memory access "
                            "violations (prevents runtime crashes).");
    MacroDefinitions.Add(Keyword, MDefinition);

    Keyword = TEXT("Enroll");
    MDefinition.Info =
        TEXT("MGC Macro.\n\nRegisters this Script as a 'Runtime Latent Node'.\nThis is a fundamental step to enable "
             "the use Magic Nodes with built-in persistent functions, such as\n\nAwake()\nStart()\nUpdate()\nStop()");
    MacroDefinitions.Add(Keyword, MDefinition);
    ...
    Keyword = TEXT("Stop");
    MDefinition.Info = TEXT("MGC Function.\n\nCalled from 'Latent Runtime Node's routines once this Script is set to "
                            "be destroyed,\nif this Script has subscribed to be latent.\n(executes only once)");
    MacroDefinitions.Add(Keyword, MDefinition);

    Keyword = TEXT("Finish");
    MDefinition.Info = TEXT("MGC Function.\n\nRequests this 'Latent Runtime Node' to be destroyed,\nignored if this "
                            "Script has never subscribed to be latent.\n(executes only once)");
    MacroDefinitions.Add(Keyword, MDefinition);
    ...
    DocsURL =
        TEXT("http://api.unrealengine.com/INT/API/Runtime/Engine/GameFramework/UPawnMovementComponent/index.html");
    Documentation.Add(Keyword, DocsURL);

    Keyword = TEXT("UCharacterMovementComponent");
    DocsURL =
        TEXT("http://api.unrealengine.com/INT/API/Runtime/Engine/GameFramework/UCharacterMovementComponent/index.html");
    Documentation.Add(Keyword, DocsURL);

    Keyword = TEXT("UProjectileMovementComponent");
    DocsURL = TEXT(
        "http://api.unrealengine.com/INT/API/Runtime/Engine/GameFramework/UProjectileMovementComponent/index.html");
    Documentation.Add(Keyword, DocsURL);

    Keyword = TEXT("URotatingMovementComponent");
    DocsURL =
        TEXT("http://api.unrealengine.com/INT/API/Runtime/Engine/GameFramework/URotatingMovementComponent/index.html");
    Documentation.Add(Keyword, DocsURL);
    ...
    DocsURL =
        TEXT("http://api.unrealengine.com/INT/API/Runtime/Engine/Components/UDirectionalLightComponent/index.html");
    Documentation.Add(Keyword, DocsURL);
    ...
    DocsURL =
        TEXT("http://api.unrealengine.com/INT/API/Runtime/Engine/PhysicsEngine/UPhysicsConstraintComponent/index.html");
    Documentation.Add(Keyword, DocsURL);

    Keyword = TEXT("UPhysicsSpringComponent");
    DocsURL =
        TEXT("http://api.unrealengine.com/INT/API/Runtime/Engine/PhysicsEngine/UPhysicsSpringComponent/index.html");
    Documentation.Add(Keyword, DocsURL);

    Keyword = TEXT("UPhysicsThrusterComponent");
    DocsURL =
        TEXT("http://api.unrealengine.com/INT/API/Runtime/Engine/PhysicsEngine/UPhysicsThrusterComponent/index.html");
    Documentation.Add(Keyword, DocsURL);
    ...
    DocsURL =
        TEXT("http://api.unrealengine.com/INT/API/Runtime/Engine/Components/UMaterialBillboardComponent/index.html");
    Documentation.Add(Keyword, DocsURL);
    ...
    DocsURL =
        TEXT("http://api.unrealengine.com/INT/API/Runtime/Engine/Components/UInstancedStaticMeshComponent/index.html");
    Documentation.Add(Keyword, DocsURL);

    Keyword = TEXT("UHierarchicalInstancedStaticMeshComponent");
    DocsURL = TEXT(
        "http://api.unrealengine.com/INT/API/Runtime/Engine/Components/UHierarchicalInstancedStaticMesh-/index.html");
    Documentation.Add(Keyword, DocsURL);
    ...
    DocsURL =
        TEXT("http://api.unrealengine.com/INT/API/Runtime/Engine/Components/UReflectionCaptureComponent/index.html");
    Documentation.Add(Keyword, DocsURL);

    Keyword = TEXT("UBoxReflectionCaptureComponent");
    DocsURL =
        TEXT("http://api.unrealengine.com/INT/API/Runtime/Engine/Components/UBoxReflectionCaptureComponent/index.html");
    Documentation.Add(Keyword, DocsURL);

    Keyword = TEXT("UPlaneReflectionCaptureComponent");
    DocsURL = TEXT(
        "http://api.unrealengine.com/INT/API/Runtime/Engine/Components/UPlaneReflectionCaptureComponent/index.html");
    Documentation.Add(Keyword, DocsURL);

    Keyword = TEXT("USphereReflectionCaptureComponent");
    DocsURL = TEXT(
        "http://api.unrealengine.com/INT/API/Runtime/Engine/Components/USphereReflectionCaptureComponen-/index.html");
    Documentation.Add(Keyword, DocsURL);
    ...
    DocsURL =
        TEXT("http://api.unrealengine.com/INT/API/Runtime/Engine/Components/UPlanarReflectionComponent/index.html");
    Documentation.Add(Keyword, DocsURL);
    ...
    DocsURL =
        TEXT("http://api.unrealengine.com/INT/API/Runtime/Engine/Components/USceneCaptureComponentCube/index.html");
    Documentation.Add(Keyword, DocsURL);
    ...
    DocsURL = TEXT(
        "http://api.unrealengine.com/INT/API/Runtime/Engine/Components/UWindDirectionalSourceComponent/index.html");
    Documentation.Add(Keyword, DocsURL);
    ...
    if (!Settings->ScanUnrealTypesOnEditorStartup)
    {
        return;
    }
...
void UMGC_SemanticDB::UpdateExtensions()
{
    ...
    for (TObjectIterator<UStruct> IT; IT; ++IT)
    {
        ...
        if (IT->IsA(UClass::StaticClass()))
        {
            continue;
        }
        if (IT->GetName().Len() > 56)
        {
            continue;
        }
    ...
    {
        if (!IT->HasAllFlags(RF_ClassDefaultObject | RF_ArchetypeObject | RF_DefaultSubObject))
        {
            continue;
        }
    
        if (IT->IsA(UEnum::StaticClass()))
        {
            continue;
        }
        if (IT->IsA(UClass::StaticClass()))
        ...
        if (IT->IsA(UStruct::StaticClass()))
        {
            continue;
        }
        if (IT->IsA(UMagicNode::StaticClass()))
        {
            continue;
        }
        if (IT->IsA(UActorComponent::StaticClass()))
        {
            continue;
        }
        if (IT->IsA(UBlueprintNodeSpawner::StaticClass()))
        {
            continue;
        }
...
void UMGC_SemanticDB::RegisterClassReflection(UClass *Class, FString Prefix)
{
    ...
    for (TFieldIterator<FProperty> PIT(Class, EFieldIteratorFlags::IncludeSuper); PIT; ++PIT)
    {
        ...
        if (PIT->HasAnyPropertyFlags(CPF_Protected))
        {
            PropDefinition.Access = EAccessLevel::Protected;
        }
        ...
        else if (Property->IsA(FObjectProperty::StaticClass()))
        {
            ...
            else if (Property->IsA(FFloatProperty::StaticClass()))
            {
                ...
                if (Property->IsA(FArrayProperty::StaticClass()))
                {
                    ...
                    else if (PArray->Inner && PArray->Inner->IsA(FClassProperty::StaticClass()))
                    {
                        PropDefinition.TypeOf = EType::Class;
                    }
                    else if (PArray->Inner && PArray->Inner->IsA(FStructProperty::StaticClass()))
                    {
                        PropDefinition.TypeOf = EType::Struct;
                    }
                    else if (PArray->Inner && PArray->Inner->IsA(FObjectProperty::StaticClass()))
                    {
                        PropDefinition.TypeOf = EType::Object;
                    }
                    else
                    
                        if (PArray->Inner && PArray->Inner->IsA(FIntProperty::StaticClass()))
                        {
                            PropDefinition.TypeOf = EType::Int;
                        }
                        else if (PArray->Inner && PArray->Inner->IsA(FBoolProperty::StaticClass()))
                        {
                            PropDefinition.TypeOf = EType::Bool;
                        }
                        else if (PArray->Inner && PArray->Inner->IsA(FByteProperty::StaticClass()))
                        {
                            PropDefinition.TypeOf = EType::Byte;
                        }
                        else if (PArray->Inner && PArray->Inner->IsA(FNameProperty::StaticClass()))
                        {
                            PropDefinition.TypeOf = EType::Name;
                        }
                        else if (PArray->Inner && PArray->Inner->IsA(FTextProperty::StaticClass()))
                        {
                            PropDefinition.TypeOf = EType::Text;
                        }
                        else if (PArray->Inner && PArray->Inner->IsA(FStrProperty::StaticClass()))
                        {
                            PropDefinition.TypeOf = EType::String;
                        }
                        else if (PArray->Inner && PArray->Inner->IsA(FFloatProperty::StaticClass()))
                        {
                            PropDefinition.TypeOf = EType::Float;
                        }
                }
                else if (Property->IsA(FSetProperty::StaticClass()))
                {
                    ...
                    else if (PSet->ElementProp && PSet->ElementProp->IsA(FClassProperty::StaticClass()))
                    {
                        PropDefinition.TypeOf = EType::Class;
                    }
                    else if (PSet->ElementProp && PSet->ElementProp->IsA(FStructProperty::StaticClass()))
                    {
                        PropDefinition.TypeOf = EType::Struct;
                    }
                    else if (PSet->ElementProp && PSet->ElementProp->IsA(FObjectProperty::StaticClass()))
                    {
                        PropDefinition.TypeOf = EType::Object;
                    }
                    else
                    
                        if (PSet->ElementProp && PSet->ElementProp->IsA(FIntProperty::StaticClass()))
                        {
                            PropDefinition.TypeOf = EType::Int;
                        }
                        else if (PSet->ElementProp && PSet->ElementProp->IsA(FBoolProperty::StaticClass()))
                        {
                            PropDefinition.TypeOf = EType::Bool;
                        }
                        else if (PSet->ElementProp && PSet->ElementProp->IsA(FByteProperty::StaticClass()))
                        {
                            PropDefinition.TypeOf = EType::Byte;
                        }
                        else if (PSet->ElementProp && PSet->ElementProp->IsA(FNameProperty::StaticClass()))
                        {
                            PropDefinition.TypeOf = EType::Name;
                        }
                        else if (PSet->ElementProp && PSet->ElementProp->IsA(FTextProperty::StaticClass()))
                        {
                            PropDefinition.TypeOf = EType::Text;
                        }
                        else if (PSet->ElementProp && PSet->ElementProp->IsA(FStrProperty::StaticClass()))
                        {
                            PropDefinition.TypeOf = EType::String;
                        }
                        else if (PSet->ElementProp && PSet->ElementProp->IsA(FFloatProperty::StaticClass()))
                        {
                            PropDefinition.TypeOf = EType::Float;
                        }
                }
                else if (Property->IsA(FMapProperty::StaticClass()))
                {
                    ...
                    else if (PMap->KeyProp && PMap->KeyProp->IsA(FClassProperty::StaticClass()))
                    {
                        PropDefinition.TypeOf = EType::Class;
                    }
                    else if (PMap->KeyProp && PMap->KeyProp->IsA(FStructProperty::StaticClass()))
                    {
                        PropDefinition.TypeOf = EType::Struct;
                    }
                    else if (PMap->KeyProp && PMap->KeyProp->IsA(FObjectProperty::StaticClass()))
                    {
                        PropDefinition.TypeOf = EType::Object;
                    }
                    else
                    
                        if (PMap->KeyProp && PMap->KeyProp->IsA(FIntProperty::StaticClass()))
                        {
                            PropDefinition.TypeOf = EType::Int;
                        }
                        else if (PMap->KeyProp && PMap->KeyProp->IsA(FBoolProperty::StaticClass()))
                        {
                            PropDefinition.TypeOf = EType::Bool;
                        }
                        else if (PMap->KeyProp && PMap->KeyProp->IsA(FByteProperty::StaticClass()))
                        {
                            PropDefinition.TypeOf = EType::Byte;
                        }
                        else if (PMap->KeyProp && PMap->KeyProp->IsA(FNameProperty::StaticClass()))
                        {
                            PropDefinition.TypeOf = EType::Name;
                        }
                        else if (PMap->KeyProp && PMap->KeyProp->IsA(FTextProperty::StaticClass()))
                        {
                            PropDefinition.TypeOf = EType::Text;
                        }
                        else if (PMap->KeyProp && PMap->KeyProp->IsA(FStrProperty::StaticClass()))
                        {
                            PropDefinition.TypeOf = EType::String;
                        }
                        else if (PMap->KeyProp && PMap->KeyProp->IsA(FFloatProperty::StaticClass()))
                        {
                            PropDefinition.TypeOf = EType::Float;
                        }
                }
    ...
    for (TFieldIterator<UFunction> FIT(Class, EFieldIteratorFlags::IncludeSuper); FIT; ++FIT)
    {
        ...
        if (FIT->HasAnyFunctionFlags(FUNC_BlueprintCallable))
        {
            FunDefinition.Flag = EFunctionFlag::Callable;
        }
        if (FIT->HasAnyFunctionFlags(FUNC_BlueprintEvent))
        {
            FunDefinition.Flag = EFunctionFlag::Event;
        }
        if (FIT->HasAnyFunctionFlags(FUNC_BlueprintPure))
        {
            FunDefinition.Flag = EFunctionFlag::Pure;
        }
        if (FIT->HasAnyFunctionFlags(FUNC_Delegate))
        {
            FunDefinition.Flag = EFunctionFlag::Delegate;
        }
        if (FIT->HasAnyFunctionFlags(FUNC_Const))
        {
            FunDefinition.Flag = EFunctionFlag::Const;
        }
        if (FIT->HasAnyFunctionFlags(FUNC_Event))
        {
            FunDefinition.Flag = EFunctionFlag::Event;
        }
    
        FunDefinition.Access = EAccessLevel::Static;
        if (FIT->HasAnyFunctionFlags(FUNC_Public))
        {
            FunDefinition.Access = EAccessLevel::Public;
        }
        if (FIT->HasAnyFunctionFlags(FUNC_Private))
        {
            FunDefinition.Access = EAccessLevel::Private;
        }
        if (FIT->HasAnyFunctionFlags(FUNC_Protected))
        {
            FunDefinition.Access = EAccessLevel::Protected;
        }
        ...
        for (TFieldIterator<FProperty> PIT(Function); PIT && (PIT->PropertyFlags & CPF_Parm); ++PIT)
        {
            ...
            {
                FunDefinition.Inputs.Add(Display);
            }
        }
...
{
    if (Struct->IsA(UFunction::StaticClass()))
    {
        return;
    }
...
FString UMGC_SemanticDB::PropertyToTypeName(TFieldPath<FProperty> Property)
{
    ...
    if (Property->IsA(FClassProperty::StaticClass()))
    {
        ...
        if (CLS != nullptr)
        {
            PType = FString(TEXT("U")) + CLS->GetName();
        }
        return PType;
    }
    ...
    if (Property->IsA(FStructProperty::StaticClass()))
    {
        ...
        if (PStruct->Struct == TBaseStructure<FColor>::Get())
        {
            PType = TEXT("FColor");
            return PType;
        }
        if (PStruct->Struct == TBaseStructure<FVector>::Get())
        {
            PType = TEXT("FVector");
            return PType;
        }
        if (PStruct->Struct == TBaseStructure<FRotator>::Get())
        {
            PType = TEXT("FRotator");
            return PType;
        }
        if (PStruct->Struct == TBaseStructure<FVector2D>::Get())
        {
            PType = TEXT("FVector2D");
            return PType;
        }
        if (PStruct->Struct == TBaseStructure<FTransform>::Get())
        {
            PType = TEXT("FTransform");
            return PType;
        }
        if (PStruct->Struct == TBaseStructure<FLinearColor>::Get())
        {
            PType = TEXT("FLinearColor");
            return PType;
        }
    ...
    if (Property->IsA(FObjectProperty::StaticClass()))
    {
        ...
        if (OBJ != nullptr)
        {
            PType = FString(TEXT("U")) + OBJ->GetName();
        }
        return PType;
    }
    ...
    if (Property->IsA(FSetProperty::StaticClass()))
    {
        ...
        if (PSet->ElementProp && PSet->ElementProp->IsA(FBoolProperty::StaticClass()))
        {
            PType = TEXT("TSet< bool >");
            return PType;
        }
        if (PSet->ElementProp && PSet->ElementProp->IsA(FByteProperty::StaticClass()))
        {
            PType = TEXT("TSet< uint8 >");
            return PType;
        }
        if (PSet->ElementProp && PSet->ElementProp->IsA(FFloatProperty::StaticClass()))
        {
            PType = TEXT("TSet< float >");
            return PType;
        }
    
        if (PSet->ElementProp && PSet->ElementProp->IsA(FNameProperty::StaticClass()))
        {
            PType = TEXT("TSet< FName >");
            return PType;
        }
        if (PSet->ElementProp && PSet->ElementProp->IsA(FTextProperty::StaticClass()))
        {
            PType = TEXT("TSet< FText >");
            return PType;
        }
        if (PSet->ElementProp && PSet->ElementProp->IsA(FStrProperty::StaticClass()))
        {
            PType = TEXT("TSet< FString >");
            return PType;
        }
        ...
        if (PSet->ElementProp && PSet->ElementProp->IsA(FStructProperty::StaticClass()))
        {
            ...
            if (PStruct->Struct == TBaseStructure<FColor>::Get())
            {
                PType = TEXT("TSet< FColor >");
                return PType;
            }
            if (PStruct->Struct == TBaseStructure<FVector>::Get())
            {
                PType = TEXT("TSet< FVector >");
                return PType;
            }
            if (PStruct->Struct == TBaseStructure<FRotator>::Get())
            {
                PType = TEXT("TSet< FRotator >");
                return PType;
            }
            if (PStruct->Struct == TBaseStructure<FVector2D>::Get())
            {
                PType = TEXT("TSet< FVector2D >");
                return PType;
            }
            if (PStruct->Struct == TBaseStructure<FTransform>::Get())
            {
                PType = TEXT("TSet < FTransform >");
                return PType;
            }
            if (PStruct->Struct == TBaseStructure<FLinearColor>::Get())
            {
                PType = TEXT("TSet< FLinearColor >");
                return PType;
            }
        ...
        if (PSet->ElementProp && PSet->ElementProp->IsA(FObjectProperty::StaticClass()))
        {
            ...
            if (OBJ != nullptr)
            {
                PType = FString::Printf(TEXT("TSet< U%s >"), *OBJ->GetName());
            }
            return PType;
        }
    }
    ...
    if (Property->IsA(FMapProperty::StaticClass()))
    {
        ...
        if (PMap->KeyProp && PMap->KeyProp->IsA(FBoolProperty::StaticClass()))
        {
            PType = TEXT("TMap< bool, ... >");
            return PType;
        }
        if (PMap->KeyProp && PMap->KeyProp->IsA(FIntProperty::StaticClass()))
        {
            PType = TEXT("TMap< int32, ... >");
            return PType;
        }
        if (PMap->KeyProp && PMap->KeyProp->IsA(FByteProperty::StaticClass()))
        {
            PType = TEXT("TMap< uint8, ... >");
            return PType;
        }
        if (PMap->KeyProp && PMap->KeyProp->IsA(FFloatProperty::StaticClass()))
        {
            PType = TEXT("TMap< float, ... >");
            return PType;
        }
    
        if (PMap->KeyProp && PMap->KeyProp->IsA(FNameProperty::StaticClass()))
        {
            PType = TEXT("TMap< FName, ... >");
            return PType;
        }
        if (PMap->KeyProp && PMap->KeyProp->IsA(FTextProperty::StaticClass()))
        {
            PType = TEXT("TMap< FText, ... >");
            return PType;
        }
        if (PMap->KeyProp && PMap->KeyProp->IsA(FStrProperty::StaticClass()))
        {
            PType = TEXT("TMap< FString, ... >");
            return PType;
        }
        ...
        if (PMap->KeyProp && PMap->KeyProp->IsA(FClassProperty::StaticClass()))
        {
            ...
            if (CLS != nullptr)
            {
                PType = FString::Printf(TEXT("TMap< U%s, ... >"), *CLS->GetName());
            }
            return PType;
        }
        ...
        if (PMap->KeyProp && PMap->KeyProp->IsA(FStructProperty::StaticClass()))
        {
            ...
            if (PStruct->Struct == TBaseStructure<FColor>::Get())
            {
                PType = TEXT("TMap< FColor, ... >");
                return PType;
            }
            if (PStruct->Struct == TBaseStructure<FVector>::Get())
            {
                PType = TEXT("TMap< FVector, ... >");
                return PType;
            }
            if (PStruct->Struct == TBaseStructure<FRotator>::Get())
            {
                PType = TEXT("TMap< FRotator, ... >");
                return PType;
            }
            if (PStruct->Struct == TBaseStructure<FVector2D>::Get())
            {
                PType = TEXT("TMap< FVector2D, ... >");
                return PType;
            }
            if (PStruct->Struct == TBaseStructure<FTransform>::Get())
            {
                PType = TEXT("TMap< FTransform, ... >");
                return PType;
            }
            if (PStruct->Struct == TBaseStructure<FLinearColor>::Get())
            {
                PType = TEXT("TMap< FLinearColor, ... >");
                return PType;
            }
        ...
        if (PMap->KeyProp && PMap->KeyProp->IsA(FObjectProperty::StaticClass()))
        {
            ...
            if (OBJ != nullptr)
            {
                PType = FString::Printf(TEXT("TMap< U%s, ... >"), *OBJ->GetName());
            }
            return PType;
        }
    }
    ...
    if (Property->IsA(FArrayProperty::StaticClass()))
    {
        ...
        if (PArray->Inner && PArray->Inner->IsA(FBoolProperty::StaticClass()))
        {
            PType = TEXT("TArray< bool >");
            return PType;
        }
        if (PArray->Inner && PArray->Inner->IsA(FIntProperty::StaticClass()))
        {
            PType = TEXT("TArray< int32 >");
            return PType;
        }
        if (PArray->Inner && PArray->Inner->IsA(FByteProperty::StaticClass()))
        {
            PType = TEXT("TArray< uint8 >");
            return PType;
        }
        if (PArray->Inner && PArray->Inner->IsA(FFloatProperty::StaticClass()))
        {
            PType = TEXT("TArray< float >");
            return PType;
        }
    
        if (PArray->Inner && PArray->Inner->IsA(FNameProperty::StaticClass()))
        {
            PType = TEXT("TArray< FName >");
            return PType;
        }
        if (PArray->Inner && PArray->Inner->IsA(FTextProperty::StaticClass()))
        {
            PType = TEXT("TArray< FText >");
            return PType;
        }
        if (PArray->Inner && PArray->Inner->IsA(FStrProperty::StaticClass()))
        {
            PType = TEXT("TArray< FString >");
            return PType;
        }
        ...
        if (PArray->Inner && PArray->Inner->IsA(FClassProperty::StaticClass()))
        {
            ...
            if (CLS != nullptr)
            {
                PType = FString::Printf(TEXT("TArray< U%s, ... >"), *CLS->GetName());
            }
            return PType;
        }
        ...
        if (PArray->Inner && PArray->Inner->IsA(FStructProperty::StaticClass()))
        {
            ...
            if (PStruct->Struct == TBaseStructure<FColor>::Get())
            {
                PType = TEXT("TArray< FColor >");
                return PType;
            }
            if (PStruct->Struct == TBaseStructure<FVector>::Get())
            {
                PType = TEXT("TArray< FVector >");
                return PType;
            }
            if (PStruct->Struct == TBaseStructure<FRotator>::Get())
            {
                PType = TEXT("TArray< FRotator >");
                return PType;
            }
            if (PStruct->Struct == TBaseStructure<FVector2D>::Get())
            {
                PType = TEXT("TArray< FVector2D >");
                return PType;
            }
            if (PStruct->Struct == TBaseStructure<FTransform>::Get())
            {
                PType = TEXT("TArray< FTransform >");
                return PType;
            }
            if (PStruct->Struct == TBaseStructure<FLinearColor>::Get())
            {
                PType = TEXT("TArray< FLinearColor >");
                return PType;
            }
        ...
        if (PArray->Inner && PArray->Inner->IsA(FObjectProperty::StaticClass()))
        {
            ...
            if (OBJ != nullptr)
            {
                PType = FString::Printf(TEXT("TArray< U%s, ... >"), *OBJ->GetName());
            }
            return PType;
        }
    }
