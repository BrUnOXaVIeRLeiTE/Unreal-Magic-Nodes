...
FText UKMGC_MagicNode::GetNodeTitle(ENodeTitleType::Type TType) const
{
    ...
    if (TType != ENodeTitleType::MenuTitle)
    {
        ...
        for (int32 I = 1; I < SName.Len(); I++)
        {
            ...
            {
                if (SName[I - 1] != TEXT(' ') && !FChar::IsUpper(SName[I - 1]))
                {
                    SCaps.AppendChar(TEXT(' '));
                }
            }
        ...
        CachedNodeTitle.SetCachedText(
            FText::Format(NSLOCTEXT("K2Node", "KMGC_MagicNode_Title", "(C++) :: {Name}"), Args), this);
...
void UKMGC_MagicNode::AllocateDefaultPins()
{
    ...
    FEdGraphPinType PNT_Exec(KSchema->PC_Exec, TEXT(""), nullptr, EPinContainerType::None, false,
                             FEdGraphTerminalType());
    FEdGraphPinType PNT_Object(KSchema->PC_Object, TEXT(""), UObject::StaticClass(), EPinContainerType::None, false,
                               FEdGraphTerminalType());
    FEdGraphPinType PNT_Script(KSchema->PC_Object, TEXT(""), UMagicNodeScript::StaticClass(), EPinContainerType::None,
    ...
    if (PIN_Finish == nullptr)
    {
        PIN_Finish = CreatePin(EGPD_Output, PNT_Exec, KSchema->PN_Then);
    }
    if (PIN_Execute == nullptr)
    {
        PIN_Execute = CreatePin(EGPD_Input, PNT_Exec, KSchema->PN_Execute);
    }

    /// Node's Context Pin.
    UEdGraphPin *PIN_Context = GetContextPin();
    if (PIN_Context == nullptr)
    {
        PIN_Context = CreatePin(EGPD_Input, PNT_Object, UKMGC_MagicNodeHelper::PN_Context);
    }

    /// Node's Script Pin.
    UEdGraphPin *PIN_Script = GetScriptPin();
    if (PIN_Script == nullptr)
    {
        PIN_Script = CreatePin(EGPD_Input, PNT_Script, UKMGC_MagicNodeHelper::PN_Script);
    }
...
{
    if (this->IsPendingKill() || !SourceGraph->IsValidLowLevelFast() || SourceGraph->IsPendingKill())
    {
        BreakAllNodeLinks();
        return;
    }
    ...
    if (ScriptObject == nullptr)
    {
        CompilerContext.MessageLog.Error(
            *LOCTEXT("KMGC_MagicNode_Script_Error", "[Magic Node]: @@ must have a @@ specified!").ToString(), Self,
            PIN_Script);
    ...
    {
        CompilerContext.MessageLog.Error(
            *LOCTEXT("KMGC_MagicNode_Function_Error",
                     "[Magic Node]: @@ unable to allocate Executable Function; Consider compiling or hot-reloading the "
                     "C++ project before binding this node.")
                 .ToString(),
            Self);
    }
    else
    {
        UK2Node_CallFunction *MGC_Execute =
            CompilerContext.SpawnIntermediateNode<UK2Node_CallFunction>(Self, SourceGraph);
        ...
        for (int32 I = 0; I < Pins.Num(); I++)
        {
            ...
            if (PUT)
            {
                CompilerContext.MovePinLinksToIntermediate(*PIN, *PUT);
            }
        }
    }
...
{
    if (ChangedPin && (ChangedPin->PinName == UKMGC_MagicNodeHelper::PN_Script))
    {
        ScriptPinChanged();
        return;
    }
...
void UKMGC_MagicNode::CreatePinsForScript(UFunction *Function)
{
    ...
    if (Function != nullptr)
    {
        ...
        {
            if (IsDefaultParam(*PIT))
            {
                continue;
            }
        
            TFieldPath<FProperty> Property = *PIT;
            if (PIT->HasAnyPropertyFlags(CPF_OutParm | CPF_ReturnParm) &&
                (!PIT->HasAnyPropertyFlags(CPF_ReferenceParm)))
            {
                if (FindPin(PIT->GetFName()) == nullptr && FBlueprintEditorUtils::PropertyStillExists(*PIT))
                {
                    if (UEdGraphPin *Pin = CreatePin(EGPD_Output, NAME_None, Property->GetFName()))
                    {
                        ...
                        {
                            KSchema->SetPinAutogeneratedDefaultValueBasedOnType(Pin);
                        }
...
UEdGraphPin *UKMGC_MagicNode::GetScriptPin(const TArray<UEdGraphPin *> *InPinsToSearch) const
{
    ...
    for (auto IT = PinsToSearch->CreateConstIterator(); IT; ++IT)
    {
        ...
        if (Source && (Source->PinName == UKMGC_MagicNodeHelper::PN_Script))
        {
            Pin = Source;
            break;
        }
...
UEdGraphPin *UKMGC_MagicNode::GetContextPin(const TArray<UEdGraphPin *> *InPinsToSearch) const
{
    ...
    {
        UEdGraphPin *Source = *IT;
        if (Source && (Source->PinName == UKMGC_MagicNodeHelper::PN_Context))
        {
            Pin = Source;
            break;
        }
...
bool UKMGC_MagicNode::IsDefaultPin(UEdGraphPin *Pin)
{
    ...
    return (Pin->PinName == KSchema->PN_Then || Pin->PinName == KSchema->PN_Execute ||
            Pin->PinName == UKMGC_MagicNodeHelper::PN_Script || Pin->PinName == UKMGC_MagicNodeHelper::PN_Context);
}
...
bool UKMGC_MagicNode::IsDefaultParam(const TFieldPath<FProperty> Param)
{
    ...
    return (Param->GetFName() == KSchema->PN_Then || Param->GetFName() == KSchema->PN_Execute ||
...
FLinearColor UKMGC_MagicNode::GetNodeTitleColor() const
{
    if (!HasScript())
    {
        return FLinearColor(FColor(5, 15, 25));
    }
    ...
    if (!RuntimeScriptObject.IsValid())
    {
        if (GetScriptObject()->RefreshRuntimeScriptClass())
        {
            ...
            if (Script->RefreshRuntimeScriptClass())
            {
                ...
                const FName CheckedID =
                    MakeUniqueObjectName(GetTransientPackage(), UMagicNode::StaticClass(), FName(*Name));
...
bool UKMGC_MagicNode::CanCompileProject() const
{
    ...
    return (HasScript() && (BP && BP->ParentClass && (!BP->bBeingCompiled)) &&
...
void UKMGC_MagicNode::CompileScript()
{
    ...
    else if (ScriptObject == UMagicNodeScript::StaticClass()->ClassDefaultObject)
    {
        GetBlueprint()->Message_Error(TEXT("{MGC}:: Compiler unreachable, Script is invalid (referencing base Magic "
                                           "Node Script Class is not allowed)."));
        return;
    }

    CompilerResult = CompileScriptClass(ScriptObject->GetName(), GetHeaderText(), GetScriptText(), GetTypesText(),
                                        GetParentClass(), GetIncludes(), GetMacros());
    if (CompilerResult != EMGC_CompilerResult::MGC_None)
    {
        ...
        case EMGC_CompilerResult::Compiled: {
            ...
            ErrorMsg = TEXT("");
            ScriptNodeWidget->SetErrorMessage(TEXT(""));
...
void UKMGC_MagicNode::ReloadRuntimeScript()
{
    ...
    {
        GetBlueprint()->Message_Error(TEXT("{MGC}:: Reload unreachable, Script is invalid (referencing base Magic Node "
                                           "Script Class is not allowed)."));
        return;
    }
